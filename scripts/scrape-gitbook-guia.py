#!/usr/bin/env python3
"""Importa guías de producto cliente desde guia.puntopago.net a content/gitbook-guia.json."""
...
BASE = "https://guia.puntopago.net"
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "content" / "gitbook-guia.json"
IMAGE_MAP_OUT = ROOT / "content" / "gitbook-guia-image-map.json"
GITBOOK_SPACE = "OHKoSH5Mw16xl5hF1d7i"

# slug de página GitBook -> (slug categoría FAQ, título, descripción, icono)
PAGE_CATEGORY_MAP: dict[str, tuple[str, str, str, str]] = {
    "pago-con-cuotas": (
        "cuotas-debito",
        "Cuotas débito",
        "Paga en cuotas con tu tarjeta de débito o virtual Punto Pago.",
        "credit-card",
    ),
    "linea-de-credito": (
        "linea-credito",
    ...
    "adelanto-de-saldo": (
        "adelanto-de-saldo",
        "Adelanto de saldo",
        "Adelanta saldo para pagar facturas y servicios desde la app Punto Pago.",
        "wallet",
    ),
    "tarjeta-de-credito": (
        "tarjeta-de-credito",
        "Tarjeta de crédito",
        "Guía oficial de la tarjeta de crédito Punto Pago, sin anualidad de por vida.",
        "credit-card",
    ),
}
...
def clean_gitbook_markdown(raw: str) -> str:
    ...
    text = re.sub(
        r"<div align=\"[^\"]*\">\s*<figure>\s*<img src=\"([^\"]+)\"[^>]*>.*?</figure>\s*</div>",
        r"![](\1)",
        text,
        flags=re.S,
    )
    ...
    text = re.sub(r"</?(figure|figcaption|div)[^>]*>", "", text)
...
def page_slug(page: dict) -> str:
    ...
    if not pathname:
        return "bienvenido-a-punto-pago"
...
def main() -> None:
    ...
    for page in pages:
        slug = page_slug(page)
        if slug not in PAGE_CATEGORY_MAP:
            continue

        cat_slug, cat_title, cat_description, cat_icon = PAGE_CATEGORY_MAP[slug]
        ...
        categories[cat_slug]["articles"].append(
            {
    ...
    order = [meta[0] for meta in PAGE_CATEGORY_MAP.values()]
    result_categories = [categories[key] for key in order if key in categories and categories[key]["articles"]]
