  ...
  if (requiresPassword && !unlocked) {
    return (
      <div className="mx-auto max-w-md">
        <h1 className="text-xl font-bold text-[#0B0B13]">{t("Revisar traducciones")}</h1>
        <p className="mt-2 text-sm text-slate-600">{t("Ingresa la clave del equipo para continuar.")}</p>
        <input
          type="password"
          value={clave}
    ...
    <div className="mx-auto max-w-6xl">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {batch?.running ? t("Traduciendo todo…") : t("Traducir todo lo que falta")}
          </button>
          <Link
            href="/redactar"
          ...
          {batch.running ? (
            <div>
              <p className="font-medium text-[#0B0B13]">
                {t("Traduciendo todas las guías pendientes…")}{" "}
                {batch.total !== null && (
                  <span className="text-slate-600">
                    {batch.translated} {t("de")} {batch.total}
                  </span>
                )}
              </p>
              ...
              <p className="mt-2 text-xs text-slate-500">
                {t("No cierres esta página. Puede tardar varios minutos según la cantidad de guías.")}
              </p>
            </div>
          ) : (
            <p className="text-emerald-800">
              {t("Listo: se tradujeron")} {batch.translated} {t("guías.")}{" "}
              {batch.failed > 0 && (
                <span className="text-amber-700">
                  {batch.failed} {t("no se pudieron traducir; puedes reintentar.")}
                </span>
              )}{" "}
              {t("Los cambios se publican tras el redespliegue de Vercel (~1 min).")}
            </p>
          )}
        </div>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[18rem_1fr]">
        ...
        <aside className="lg:sticky lg:top-4 lg:max-h-[80vh] lg:self-start lg:overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {grouped.length === 0 && (
              <p className="px-1 text-sm text-slate-400">{t("Sin resultados.")}</p>
            )}
          </div>
        </aside>
        ...
        <section>
          {!selected ? (
            <div className="rounded-2xl border border-dashed border-slate-300 bg-white/60 px-6 py-16 text-center text-slate-500">
              {t("Elige una guía de la lista para empezar.")}
            </div>
          ) : loading ? (
            ...
            <>
              ...
              {saved && (
                <div className="mb-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                  {t("Traducción guardada. Se publicará tras el redespliegue de Vercel (~1 min).")}
                </div>
              )}
